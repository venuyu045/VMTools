...
public class WaypointScreen extends Screen {
    ...
    protected void init() {
        ...
        ).bounds(btnStartX + (btnWidth + btnSpacing) * 4, btnY, btnWidth, BUTTON_HEIGHT).build());

        this.addRenderableWidget(Button.builder(
                Component.literal("Back"),
                button -> sendBackCommand()
        ).bounds(btnStartX + (btnWidth + btnSpacing) * 5, btnY, btnWidth, BUTTON_HEIGHT).build());
    ...
    private void renderGroupWindow(GuiGraphicsExtractor context, WaypointGroup group, GroupWindowState state, int mouseX, int mouseY) {
        ...
        if (state.expanded) {
            ...
            for (int wpIndex = 0; wpIndex < group.getWaypoints().size(); wpIndex++) {
                ...
                // 渲染路径点行
                boolean isWpHovered = mouseX >= hoverStartX && mouseX <= x + w - 4 &&
                ...
                if (wpBg != 0x00000000 && !isDraggingThis) {
                    fillRoundedRect(context, hoverStartX, waypointY, w - (hoverStartX - x) - 4, WAYPOINT_ROW_HEIGHT, wpBg);
                }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (button == 0) {
            ...
            if (mouseX >= aeTabX && mouseX <= aeTabX + tabWidth &&
                    mouseY >= tabY && mouseY <= tabY + tabHeight) {
                this.minecraft.gui.setScreen(new AutoEscapeScreen());
                return true;
            }
        }
    ...
    private void confirmDeleteGroup(WaypointGroup group) {
        ...
        String groupId = group.getId();
        this.minecraft.gui.setScreen(ConfirmScreen.createDeleteConfirm(
                this,
                "分组 \"" + group.getName() + "\"",
                () -> {
    ...
    /**
     * 发送 Back 命令 — 传送回 /res tp 之前的位置
     * 前提：传送前已通过 /edithome resback relocate 保存了坐标
     */
    private void sendBackCommand() {
        ...
        mc.getConnection().sendCommand("home resback");
    ...
    private void openAddWaypointScreen() {
        ...
        this.minecraft.gui.setScreen(new EditWaypointScreen(this, null, targetGroup));
    }
    ...
    private void openEditWaypointScreen(Waypoint waypoint, WaypointGroup group) {
        if (waypoint != null) {
            this.minecraft.gui.setScreen(new EditWaypointScreen(this, waypoint, group));
        }
    }
    ...
    private void openEditGroupScreen(WaypointGroup group) {
        if (group != null) {
            this.minecraft.gui.setScreen(new EditGroupScreen(this, group));
        }
    }
    ...
    private void openAddGroupScreen() {
        this.minecraft.gui.setScreen(new EditGroupScreen(this, null));
    }
    ...
    private void doImport(Path selectedFile) {
        ...
        this.minecraft.execute(() -> {
            if (importedGroups != null && !importedGroups.isEmpty()) {
                this.minecraft.gui.setScreen(new ImportConfirmScreen(this, importedGroups));
            } else {
