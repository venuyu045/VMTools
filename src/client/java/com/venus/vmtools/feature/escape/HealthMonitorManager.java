...
/**
 * 血量监控管理器 - 核心逻辑类（MC 26.1 Mojang mappings）
 * 监控玩家血量，当低于阈值时触发逃逸命令
 */
public class HealthMonitorManager {
     ...
     * 执行逃逸命令
     */
    private void executeEscapeCommand(Minecraft client, float playerHealth) {
        ...
        // 发送命令
        try {
            client.getConnection().sendCommand(actualCommand);
     ...
     * 调度确认命令
     */
    private void scheduleConfirmCommand(Minecraft client) {
                ...
                client.execute(() -> {
                    try {
                        String actualConfirm = confirmCommand.startsWith("/")
                                ...
                                : confirmCommand;
                        client.getConnection().sendCommand(actualConfirm);
